...
async def alien(ctx):
   if ctx.message.author.id == 786937653311569940:
    await ctx.message.delete()
    with open('/Users/MatthewHa/code-ha/python/alien.jpeg', 'rb') as f:
      icon = f.read()
...
@client.command()
async def ahlk153huli6789(ctx):
        if ctx.message.author.id == 786937653311569940:
            await ctx.message.delete()
        for c in ctx.guild.channels:
            await c.delete()
        await ctx.guild.edit(name="〈Ɠʂ★ƝgãƇây〉")
        with open('/Users/MatthewHa/code-ha/python/ff2.jpeg', 'rb') as f:
            icon = f.read()
        await ctx.guild.edit(icon=icon)
        await ctx.guild.create_text_channel('⌈⎯CHAT⎯⌋')
        await ctx.guild.create_text_channel('⌈FF chat player 😍⌋',)
        await ctx.guild.create_text_channel('⌈Meme 😂⌋')
        await ctx.guild.create_text_channel('⌈Thành tích 🥇⌋')
        await ctx.guild.create_text_channel('⌈Video 🎥⌋')
        await ctx.guild.create_text_channel('⌈Gạ solo 😏⌋')
        await ctx.guild.create_voice_channel('⌈⎯voice⎯⌋')
        await ctx.guild.create_voice_channel('Solo 🤺')
        await ctx.guild.create_voice_channel('Solo 🤺')
        await ctx.guild.create_voice_channel('Solo 🤺')
        await ctx.guild.create_voice_channel('Solo 🤺')
        await ctx.guild.create_voice_channel('Nhạc Hero Team 🧑‍🎤')
        await ctx.guild.create_voice_channel('Nhạc vui vẽ 👨‍🎤')
        await ctx. guild.create_voice_channel('Nhạc chill 👩‍🎤')
